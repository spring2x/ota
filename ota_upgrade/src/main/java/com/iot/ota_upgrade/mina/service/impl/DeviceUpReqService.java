...
public class DeviceUpReqService extends BasicDeviceActionService {
	...
	public void process(IoSession session, BasicMessage basicMessage, JSONObject result) throws Exception {
		...
		if (!upgradeProperty.getDefaultTerminalValideCode().equals(authCode)) {
			...
			try {
				//logger.debug("*******************************   " + "device token valid start....");
				//logger.debug("*******************************   " + "device token valid param: " + params.toJSONString());
				boolean validResult = deviceTokenService.validDeviceToken(params, result);
				//logger.debug("*******************************   " + "device token valid end");
				//logger.debug("*******************************   " + "device token valid result: " + validResult);
				if (!validResult) {
					result.clear();
		...
		if (!fileValideCodeMap.containsKey(fileMark)) {
			InitDownLoadFileSchedule.fileMarkSet.add(fileMark);
			
			int testTime = 0;
			while(!fileValideCodeMap.containsKey(fileMark) && testTime < upgradeProperty.getFileInitTestNum()){
				testTime ++;
				Thread.sleep(upgradeProperty.getFileInitTestInterval());
			}
		...
		byte messageLenth = (byte) (DeviceUpReqConstant.RESPONSE_PACKAGE_SIZE_LENTH
					+ DeviceUpReqConstant.RESPONSE_SPLIT_PACKAGE_LENTH
					+ upgradePackageValidCode.length());
		//升级包所占字节数
		int packageSize = FileUtil.fileMap.get(fileMark).length;
		
		//分包个数
		short splitPackageNum = (short) (packageSize % (singlePackLength * upgradeProperty.getSinglePackageLenthUtil()) == 0 ? 
	 ...
	 * @param splitPackageNum	升级包的分包数
	 * @param upgradePackageValidCode	文件校验码
	 * @return 消息校验和
	 */
